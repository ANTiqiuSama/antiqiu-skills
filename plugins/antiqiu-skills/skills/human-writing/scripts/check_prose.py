...
OVERHEDGE_PATTERNS = (
    re.compile(r"可能[^。！？!?\n]{0,24}(?:或许|也许|大概|潜在地|一定程度上)"),
    re.compile(r"(?:或许|也许|大概|潜在地|一定程度上)[^。！？!?\n]{0,24}可能"),
    re.compile(r"可以[^。！？!?\n]{0,12}可能"),
)
...
def main() -> int:
    ...
    vague_attributions = non_overlapping_terms(prose, VAGUE_ATTRIBUTIONS)
    if vague_attributions:
        samples = "、".join(dict.fromkeys(term for _, term in vague_attributions))
        lines = "、".join(
            dict.fromkeys(
                str(line_number(text, position))
                for position, _ in vague_attributions[:8]
            )
        )
        warnings.append(
            f"模糊归因 {len(vague_attributions)} 处，第 {lines} 行出现 {samples}。"
            "补准具体来源，无法核验时保留真实不确定性或删除断言，不能编造权威。"
        )

    promotional_matches = non_overlapping_terms(prose, PROMOTIONAL_PHRASES)
    if promotional_matches:
        samples = "、".join(dict.fromkeys(term for _, term in promotional_matches))
        lines = "、".join(
            dict.fromkeys(
                str(line_number(text, position))
                for position, _ in promotional_matches[:8]
            )
        )
        warnings.append(
            f"宣传或抬价词 {len(promotional_matches)} 处，第 {lines} 行出现 {samples}。"
            "品牌文案可按目的保留，事实稿改用具体功能、动作或结果托住。"
        )

    chat_matches = non_overlapping_terms(prose, CHAT_RESIDUE)
    if chat_matches:
        samples = "、".join(dict.fromkeys(term for _, term in chat_matches))
        lines = "、".join(
            dict.fromkeys(
                str(line_number(text, position)) for position, _ in chat_matches[:8]
            )
        )
        warnings.append(
            f"疑似聊天或模型声明残留 {len(chat_matches)} 处，第 {lines} 行出现 {samples}。"
            "交付物本来就是聊天消息时保留，否则删除。"
        )

    overhedges = all_matches(prose, OVERHEDGE_PATTERNS)
    for match in overhedges[:4]:
        warnings.append(
            f"一句内限定过密，第 {line_number(text, match.start())} 行，"
            f"“{excerpt(match.group(), 44)}”。保留表达证据边界所需的一层。"
        )

    tail_start = max(0, len(prose) - 400)
    generic_conclusions = [
        (tail_start + position, phrase)
        for position, phrase in non_overlapping_terms(
            prose[tail_start:], GENERIC_CONCLUSIONS
        )
    ]
    if generic_conclusions:
        samples = "、".join(dict.fromkeys(term for _, term in generic_conclusions))
        warnings.append(
            f"结尾附近出现模板化乐观判断。{samples}。"
            "回到最后一个已知动作或结果，没有事实就提前结束。"
        )

    bold_spans = list(re.finditer(r"\*\*[^*\n]{1,80}\*\*", text))
    bold_limit = max(5, total_han // 250)
    if len(bold_spans) > bold_limit:
        warnings.append(
            f"粗体短语共 {len(bold_spans)} 处，当前提醒线为 {bold_limit} 处。"
            "教程和报告可保留必要层级，普通文章检查是否在机械强调。"
        )
    ...
    print(
        ...
        f"模糊归因 {len(vague_attributions)}，宣传词 {len(promotional_matches)}，"
        f"聊天残留 {len(chat_matches)}，过度限定 {len(overhedges)}，"
        f"模板结尾 {len(generic_conclusions)}"
    )
