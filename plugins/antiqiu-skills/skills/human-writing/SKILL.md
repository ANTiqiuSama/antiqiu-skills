---
name: human-writing
description: 创作或深度改写自然、有依据、有中文韵律的文章与叙事。用于知乎长答、论坛帖、公众号、博客、评论、人物与行业稿、科普、故事、对白、口播和演讲，以及明确要求中文去 AI 味或活人感的改稿；保留事实、原意和语气，不补造来源或亲历。普通校对、技术文档、结构化报告和聊天回复使用 refine-text。
---
...
职责边界很简单。`refine-text` 负责保真、逻辑和结构，本 Skill 负责自然中文作品的材料、叙述位置和表达。两者同时使用时，先锁定原意和事实，再调整中文表达；用户指定的格式与事实边界始终优先。
...
已有文本的保真改写不受“五件材料才能开长稿”限制，因为原稿本身就是待处理材料。先在内部记下不能改动的事实、数字、引语、立场、叙述人称与语气，再做删改。原稿里的事实依据不足时，可以保留其不确定性、删除无依据的断言、追问或核验，不能为了让文字显得具体而补造来源、数据、亲历和感受。
...
- 用户要求润色、重写、审阅现有文本或去除 AI 痕迹时，把输入视为已有初稿，先读取 `references/revision.md`；明确涉及 AI 去痕、模型腔诊断或人性化改写时，再读 `references/humanizing.md`。现实事实可能被改动时同时读取 `references/reality.md`，虚构作品则读取 `references/fiction.md`。

不要为了保险一次加载全部参考文件，只读当前任务确实命中的部分。真正的混合任务可以读取多个文件，`references/revision.md` 仍留到初稿以后。已有文本视为初稿，可以直接进入改稿流程。创作时以正向目标为主，已经写在本文件里的交付禁令留到初稿完成后再逐项检查。
...
4. 现有文本改写时，原文中用户没有授权改变的含义、事实、立场、人称与语气。
5. `references/forum-prose.md` 与本文件里的默认散文写法。
6. `references/revision.md`、`references/humanizing.md` 的改稿规则与检查脚本的提醒。
...
脚本警告只提示可疑形状。模糊归因、宣传词、聊天残留、过度限定、粗体和模板结尾都要结合文体判断，不能为了清零警告破坏教程结构、品牌语气、人物对白或用户原意。
