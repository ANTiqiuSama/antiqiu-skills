...
- **处理**：并入 `plan-work`。保留按风险选择“无计划、短清单、详细计划”的判断，并明确“迁移”这个名称本身不能证明需要迁移框架；单个本地消费者默认只设计当前数据需要的目标 schema、一次转换、一个运行路径和可恢复备份。
...
- **作用**：保真地润色、压缩、扩写、重组和综合已有材料；按需处理 README、运行手册、API 文档、错误信息和工程方案等中文技术材料。
- **真实价值**：先锁定事实、约束、立场、引用和不确定性，再优化逻辑、结构和表达；技术模式额外统一术语，明确条件与动作关系，并按真实的失败、重试、回滚或验证边界拆分步骤。
- **重复**：与 `human-writing` 都能处理中文改稿，与 `write-action-first` 都强调结论靠前，也与本次检查的 `ste-cn` 覆盖同一类技术文档。
- **潜在冲突**：如果自然中文风格规则先于保真处理，可能改变原意；如果行动优先规则作用于文稿本身，可能破坏文体。
- **处理**：保留。描述明确限定“已有文本和保真变换”；`ste-cn` 的有效部分作为按需参考进入这里，不占用新的启动项；自然中文作品在完成含义与结构阶段后才交给 `human-writing`；`write-action-first` 只整理外围回复。
...
- Antigravity 及兼容 Agent 目录中发现 4 份启用副本，正文逐字节相同，目录内都只有 `SKILL.md`。
- 文件没有记录上游仓库、安装清单或许可证；本机元数据和公开精确短语检索都不足以确认来源。
- Codex 当前没有单独安装 `ste-cn`，公开总库也没有同名 Skill。

因此本次只吸收一般方法，不复制来源不明的表达，也不新增第三方许可声明。

### 保留并重写的能力

- 同一概念使用稳定名称，优先保留正式产品名、API 字段和代码字面量；
- 使用具体动词，条件贴近动作，责任歧义真实存在时再明确执行者；
- 解释与操作步骤分开；仅在失败、重试、回滚、责任或验证边界不同的时候拆开动作；
- 删除无证据的自证结论、空泛开场、名词化套话和会制造歧义的代词；
- 增量更新先回看受影响的旧章节，同时修正过时、矛盾、重复和交叉引用不一致。

### 没有保留的规则

- 不把中文句子、步骤或段落锁定到固定字符数。字符数不能直接证明可理解或可执行，还可能迫使模型删掉条件和限定词。
- 不把这套方法称为 ASD-STE100 的中文版本、实现或认证。ASD-STE100 的正式名称就是 Simplified Technical English；本总库只借用“受控表达减少歧义”这一通用思路。
- 不全面禁止被动句、冒号、列表或某类句式。格式是否使用，由事实关系和读者任务决定。
- 不增加评分表、逐句审计或重复检查。保真、可执行和术语一致达到后即停止。
...
领域 Skill 可以和阶段型 Skill 配合，但职责不交叉。例如“精简并落实 AGENTS.md”可以由 `trim-agent-instructions` 判断每条规则，再由 `execute-work` 完成多步修改与验证；中文技术文档直接由 `refine-text` 保真改写，只有文章或叙事明确需要自然中文韵律时才交给 `human-writing`。不需要让相邻 Skill 各自重新分析同一份材料。
